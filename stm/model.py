...
class PltScore(ScoreTransformModel):
    ...
    def run(self):
        ...
        for i, col in enumerate(self.cols):
            ...
            self.raw_score_real_max = self.indf[col].max()
            self.raw_score_real_min = self.indf[col].min()
...
class GradeScoreTai(ScoreTransformModel):
    ...
    def set_para(self,
        ...
        if isinstance(mode_score_paper_max, int):
            if len(self.cols) > 0:
                if mode_score_paper_max >= max([max(self.indf[f]) for f in self.cols]):
                    self.mode_score_paper_max = mode_score_paper_max
                else:
    ...
    * from 09-17-2017
    ...
    def run(self):
        ...
        for f in self.__cols:
            ...
            tempdf = self.indf
            tempdf.loc[:, f] = tempdf[f].apply(mapi.round45r)
            ...
            outdf[f + '_percent'] = \
                outdf[f + '_sum'].apply(lambda x: mapi.round45r(x/maxsum, self.__percent_decimal))
            if self.__display:
