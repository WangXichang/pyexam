...
def test_hainan(num=1):
    if num == 1:
        ...
        ht1a = modelbase.run(name='hainan', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='ascending')
        ht1d = modelbase.run(name='hainan', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='descending')
        # use model60-300
        # score_order=='ascending', out_score_min==
        ht2a = modelbase.run(name='hainan2', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='ascending')
        ht2d = modelbase.run(name='hainan2', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='descending')
