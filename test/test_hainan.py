...
from stm import modelapp, modeltest
...
def test_hainan(num=1):
    if num == 1:
        ...
        test_data1 = modeltest.TestData(mean=60, std=14, size=60000)
        # use model100-900
        # score_order=='ascending', out_score_min==277, max==900, second_max=784
        #              'descending', out_score_max==784, second_min==101, 110, 123
        ht1a = modelapp.run(name='hainan', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='ascending')
        ht1d = modelapp.run(name='hainan', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='descending')
        # use model60-300
        # score_order=='ascending', out_score_min==
        ht2a = modelapp.run(name='hainan2', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='ascending')
        ht2d = modelapp.run(name='hainan2', df=test_data1.df, cols=['km1', 'km2'], mode_score_order='descending')

    if num == 2:
        # data2
        test_data1 = modeltest.TestData(mean=50, std=14, size=60000)
        # use model100-900
