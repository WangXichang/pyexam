...
def test_hainan(num=1):
    ...
    if num == 1:
        ...
        r1 = main.run(model='hn300', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='a', verify=True)
        r2 = main.run(model='hn300', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='d', verify=True)
        # use model60-300
        # score_order=='a', out_score_min==
        r3 = main.run(model='h300plt1', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='a', verify=1)
        r4 = main.run(model='h300plt1', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='d', verify=1)
    ...
    if num == 2:
        ...
        r1 = main.run(model='h300', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='a', verify=1)
        r2 = main.run(model='h300', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='d', verify=1)
        # use model60-300
        # score_order=='a', out_score_min==69,73    max==300(100, 9), second_max==288(99, 5)
        #              'd', out_score_max==288, second_min==60, 69, 73
        r3 = main.run(model='h300plt2', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='a', verify=1)
        r4 = main.run(model='h300plt2', df=test_data1.df, cols=['km1', 'km2'], mode_sort_order='d', verify=1)
